...
lab の pin は記録の対象そのものなので、advisory が出ても黙って上げないでください。
上げられない理由をその lab の `README.md` に書き、読み手が再現前に分かるようにします。
上げてよいのは、**上げた後の生成物がバイト一致すると示せるとき**だけです。示せたら上げて、
その旨を `README.md` に書きます。示せないなら、測り直すのではなく新しい lab としてやり直します。
...
- 同じテーマを継続する研究では、後の lab の README から、前提となる過去の lab や
  関連文書へ相対リンクを張り、シリーズを追えるようにする。
  各 lab を単体で完結させる原則は変えず、リンクは参照にとどめる
