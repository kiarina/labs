...
この検証は単一の 2 話者音声に対する動作確認であり、精度評価用の正解ラベルや
複数音声による評価、生成 WAV の聴取評価は含みません。したがって、交互の
インデックスが実際の話者交代と一致するかは未確認です。`overlap` は上位 2 話者の
確率差が小さいフレームを表しますが、実際の重複発話であることを保証しません。
