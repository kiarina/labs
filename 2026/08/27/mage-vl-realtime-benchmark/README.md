...
**M1 Max はどの条件でも継続入力に追いつきませんでした。** 最良でも codec・8 秒の RTF
`1.425` です。Mac Studio M4 Max が codec・4 秒で `0.898`、8 秒で `0.744` を出したのとは
結論が変わります。両機の RTF 比は全 8 条件で 1.67〜1.92 倍に収まっており、機種差は
条件によらずほぼ一定でした。
...
- 再エンコードした bit cost は、カメラ本来の圧縮ではなく我々のエンコーダの出力である。
  ブラウザの `MediaRecorder` で本物の圧縮ストリームを送れば結果が変わる可能性があり、未検証
